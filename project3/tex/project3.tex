...
\textcolor{red}{Vikenes: Kommentere at $t_1$ gir lavere MSE enn $t_2$ for NN. Kan sees fra differansegrafen.}

One noteworthy aspect of the neural network results, was that the MSE at $t_1=0.1$ was three times lower than it was at $t_2=0.5$. This is easily seen from figure \ref{fig:NN_relative_error}, where the difference between the numerical and analytical solution is distributed more evenly between positive and negative values for $t_1$. The reason for this however is not apparent. One possible explanation might be that as the network trains and it converges to the analytical solution, the gradients at larger $t$ values are smaller, adjusting the corresponding    

\textcolor{red}{1: Ikke samme bias for NN som for FE. 2: Lavere MSE og diff for FE(kommentar - diffusjon er jævla snill). 3: Potential. 4: Konvergenstid}
