...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.75\linewidth]{../output/plots/error_FE_x_dx_0.01.pdf}
...
All simulations so far have been conducted for a symmetric $3\times 3$ matrix. It is of interest to compare the two methods for a higher dimensional matrix as well. This is an important step towards generalization of the ability of forward euler and neural network to find eigenvalues of symmetric matrices of arbitrary shape. Figure \ref{fig:eigvec_T2_N1000_dim6} and Figure \ref{fig:eigval_T2_N1000_dim6} simulates for a $6\times 6$ symmetric matrix. The plot shows that neither forward euler nor neural network have problems with convergence. The only remarkable difference is that the predicted components of the eigenvector are slightly positively biased for the neural network, whereas for forward euler they converge perfectly. The stochastic initialization of the neural network will yield different results for different simulations. Hence, one should expect the average among multiple simulations to give a better approximation to the true eigenvector. Nonetheless, the rayleigh quotient converges to the largest eigenvalue, with third and forth decimal precision for neural network and forward euler, respectively. The simulation illustrates that forward euler is more consistent for small time steps $\Delta t$, even for higher dimensional matrices.
