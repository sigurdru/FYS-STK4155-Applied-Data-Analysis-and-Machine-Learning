...
All neurons in all hidden layers are activated by the same activation function. The input layer is just the input, so it is not activated. In the case of regression, then the output layer is not activated. Then the function value is just the weighted sum from the last hidden layer. In the case of classification, we want the values in the output nodes to indicate the probabilities. Thus, we use an activation function that considers the value in the other nodes in the other output nodes. The softmax function does this.

\begin{equation*}
	\sigma(z_i) = \frac{e^{z_i}}{\sum_{j=1}^Ke^{z_j}},
\end{equation*}
where $K$ is the number of classes, or output nodes.
...
where OLS regression is obtained by setting $\lambda=0$. There are multiple parameters to consider for stochastic gradient descent, and the ones we will study are the choice of learning rates, $\eta$, number of epochs, $N_e$, number of minibatches, $N_B$, regularization parameter, $\lambda$ and the momentum parameter $\gamma$.

We begin by studying the MSE for different values of $\eta$ as a function of $N_e\in[0,\,150]$, setting $\lambda=\gamma=0$ and $N_B=20$, i.e. minibatches of size $36$ for the $720$ training data.
\sn{skal vi kommentere her at best eta påvirkes av batch size, eller skal det være i diskusjonen?}
We choose $21$ evenly spaced values of $\eta\in[0.01,\,0.9]$. We expect higher learning rates to produce fluctuating MSE values for different epochs. Based on this we will make an additional plot with $21$ new $\eta$ values for which the MSE looks more stable.   

By making some initial parameter choices we simplify our analysis, as studying all the parameters at once are a tedious process. We then choose one of the favorable learning rates to study the MSE for minibatch sizes of $M\in[720,\,360,\,240,\,144,\,72,\,48,\,36,\,30,\,24]$, as a function of epochs, using $N_e\in[0,\,150]$ once again. Using $N_B=20$ minibatches, we then study the MSE after $N_e=150$ epochs as a function of $\eta$ and $\lambda$ to study the effect of regularization. For this we use $11$ evenly distributed values of $\eta\in[0.1,\,0.5]$ and $11$ logarthimcally distributed values of $\lambda\in[10^{-5},\,1]$. Finally, we study the MSE over $100$ epochs for $20$ evenly spaced values of $\gamma\in[0,\,0.95]$
\sn{Burde man kanskje lagd tabell med parameterne?}.

After these parameters are studied, we will test an implementation of scaling the learning rate by reducing it from an initial value after each epoch. One possible advantage of this is that convergence is fastinitially, when the errors are large, but as we approach a minima the learning rates are reduced, preventing us from overshooting. The algorithm we use for scaling the learning rate is given in equation \eqref{eq:learning_schedule}, where $t$ denotes the epoch
...
For the first simulation we choose $21$ linearly distributed values of $\eta\in[10^{-3},\,0.9]$. We will then repeat this analysis using $21$ new $\eta$ values on an interval where the resulting MSE was low. 
...
Having only studied different learning rates and regularization parameters of the neural network, we are now ready to test different configurations of the network itself. The first modification we will make is testing the neural network using different activation functions for the hidden layers. Combined with this, we will apply different initialization methods of the weights and biases \sn{biases har vi vel ikke gjort}. 
...
We proceed by plotting the test MSE for $\eta\in[0.01,\,0.5]$, shown in figure \ref{fig:SGD_Franke_epochs_eta}, where we set the maximum MSE to $0.07$ once again.
...
% KOMMENTER PLOTTET OVER 

The MSE as a function of epochs for different number of minibatches is shown in figure \ref{fig:SGD_Franke_epochs_minibatches}. The figure shows the MSE for the train and test data in the left and right panel, respectively. We use $MSE_\mathrm{max}=0.07$ once again. 

\begin{figure}[h!]
	\minipage{0.49\textwidth}
	\includegraphics[width=\linewidth]{SGD_Franke/reg_Franke__epochs_minibatches__Train_MSE__136234.pdf}
	\endminipage
	\minipage{0.49\textwidth}
	\includegraphics[width=\linewidth]{SGD_Franke/reg_Franke__epochs_minibatches__Test_MSE__440533.pdf}
	\endminipage
	\caption{SGD}
...
We plot the MSE for train and test with different regularization parameters, shown in the left and right panel of figure \ref{fig:SGD_Franke_lambda_eta}

\begin{figure}[h!]
	\minipage{0.49\textwidth}
	\includegraphics[width=\linewidth]{SGD_Franke/reg_Franke__lambda_eta__Train_MSE__866968.pdf}
	\endminipage
	\minipage{0.49\textwidth}
	\includegraphics[width=\linewidth]{SGD_Franke/reg_Franke__lambda_eta__Test_MSE__764274.pdf}
	\endminipage
	\caption{SGD}
...
Figure \ref{fig:SGD_Franke_epochs_gamma} shows the resulting MSE for train and test data for different momentum parameters as a function of epochs. 

\begin{figure}[h!]
	\minipage{0.49\textwidth}
	\includegraphics[width=\linewidth]{SGD_Franke/reg_Franke__epochs_gamma__Train_MSE__623867.pdf}
	\endminipage
	\minipage{0.49\textwidth}
	\includegraphics[width=\linewidth]{SGD_Franke/reg_Franke__epochs_gamma__Test_MSE__597266.pdf}
	\endminipage
	\caption{SGD}
