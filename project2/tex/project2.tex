...
Lastly, we study the regularization parameter $\lambda$. Using the same $\eta$ values as in Figure \ref{fig:NN_Franke_epochs_eta_MSE_limited}, and sigmoid as the activation function, we plot the test MSE after 100 epochs for 11 logarithmically distributed values of $\lambda$ between $10^{-5}$ and $1$. This is shown in Figure \ref{fig:NN_lambda}. We see that we get better result for lower $\lambda$, strictly.


\begin{figure}[h]
	\includegraphics[width=\linewidth]{plots/NN_Franke__eta_lambda__test_MSE__489556.pdf}
