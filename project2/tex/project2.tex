...
\end{align}
For Leaky RELU, $\alpha$ is some parameter for which we use $\alpha = 0.01$.
...
All neurons in all hidden layers are activated by the same activation function. The input layer is just the input, so it is not activated. In the case of regression, neither the outputl layer is activted. Then the function value is just the weighted sum from the last hidden layer. In the case of classifaction, we want the values in the output nodes to indicate the probabilities. Thus, we use an activation function that consideres the value in the other nodes in the other output nodes. The softmax function does this.
