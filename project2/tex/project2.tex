...
We now study how different activation functions affect the results. We keep using the dynamic learning rate. This time, we have to use lower learning rate, for both RELU and Leaky RELU, choosing 11 values between $10^{-6}$ and $10^{-1}$. Higher learning rates lead to overflow in the gradients. These activation functions are also able to converge to good results within 100 epochs for lower learning rates than sigmoid. This is what we saw in the regression case. However, at the end of training, the best result achieved for both functions were $97.3\%$, 3 mistakes.
