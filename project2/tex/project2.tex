...
Before we can study Franke Function we have to initialize the data. We will use the same parameters as in a previous project, such that we can compare results. One can find the previous project on our GitHub\footnote{https://github.com/sigurdru/FYS-STK4155/tree/main/project1} with all the code and instructions on how to run it. In that project we initialized the data with $N=30\times30$ randomly (uniformly) distributed datapoints in $x$- and $y$-direction. To simulate it being real data, we also added normally distributed noise with mean zero and standard deviation $\epsilon = 0.2$ ($N(0,\epsilon)$). In the previous project we, among other things, used ordinary least squares (OLS) and ridge regression to fit the Franke function. Information on those methods can be found in the aforementioned report. We can then compare the optimal MSE's in that project, with those obtained in this.

\subsubsection{Stochastic Gradient Descent}
As mentioned we want to fit the Franke function \eqref{eq:FF} using stochastic gradient decent (algorithm \eqref{eq:SGD_algo}). Writing our own code, we perform an analysis of the MSE (equation \eqref{eq:MSE}) for different learning rates $\eta$. The cost function used is ordinary MSE. We also implement $L_2$ regularization \eqref{eq:L2}, and plot the MSE's for different values of hyper-parameters $\lambda$.\sn{Scaling the learning rate}
