...
We begin by studying the MSE of the Franke function. For the MSE values we will include the result from both the train and test data in our first simulation only, in order to confirm that overfitting takes place eventually, while focusing on the test MSE in the remining simulation, which is the actual quantity of interest.

The first MSE values we calculate are for different values of $\eta$ as a function of $N_e\in[0,\,150]$, setting $\lambda=\gamma=0$ and $N_B=20$, i.e. minibatches of size $36$ for the $720$ training data. 
...
After these parameters are studied, we will test an implementation of scaling the learning rate by lowering it for each epoch. One advantage of scaling the learning rate is that we can use a large learning rate initially when the MSE is high and we are far from the minima. As we approach minima the learning rates are diminished and the probability of surpassing minima of the cost function is greatly reduced. The algorithm we implement for scaling the learning rate is given in equation \eqref{eq:learning_schedule}, where $t\in[0,\,N_e]$ denotes the current epoch. 

Our goal is the reach the actual global minima of the cost function, and by choosing a scaling algorithm 
