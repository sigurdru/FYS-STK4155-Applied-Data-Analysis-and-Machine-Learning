...
(with a logarithmic y-axis. Using a data set with \$50\textbackslash\{\}cross50\$ data points yields a little noticeable change in the mean squared error, but we see a potential overfitting occuring for \$P>15\$. To see that we get overfitting for the terrain data eventually, we plot the mean squared error for the same resampling method, but with \$n=10\$ points in each direction up to a polynomial degree of \$P=10\$, shown on the right panel in figure \textbackslash\{\}ref\{fig:terrain\_OLS\_MSE\_bootstrap\}. Here, we can see an evident overfitting starting to occur for polynomial degrees greater than \$6\$.)<-- GAMMEL ANALYSE
