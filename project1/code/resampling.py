...
def NoResampling(X, z, ttsplit, unused_iter_variable, lmd_range, reg_method, scaler):
    ...
    data = {}
    data["test_MSE"] = utils.MSE(z_test, z_pred)
...
def Bootstrap(X, z, ttsplit, B, lmd_range, reg_method, scaler):
    ...
    data["test_MSE"] = utils.MSE_boot(z_test, z_pred)
    data["test_bias"] = utils.Bias(z_test, z_pred)
    data["test_variace"] = utils.Variance(z_test, z_pred)
    return data
